...
class AtmCollectionNewPage extends StatefulWidget {
  final RequestEntity request;

  const AtmCollectionNewPage({
    super.key,
    required this.request,
  });

  @override
  State<AtmCollectionNewPage> createState() => _AtmCollectionNewPageState();
}

class _AtmCollectionNewPageState extends State<AtmCollectionNewPage> {
    ...
    final result = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => CashCountPage(
          ...
          isAtmCashCount: true, // This is ATM collection, so use ATM cash count
          onConfirm: (cashCount) {
  ...
  Future<void> _saveCashCountToServer() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('${AppConstants.baseUrl}/atm-cash-counts'), // Use ATM cash count endpoint
        headers: {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('ATM Collection Service'),
          ...
          children: [
            Row(
              children: [
                ...
                const Text(
                  'ATM Collection Request Details',
                  style: TextStyle(
                    fontSize: 16,
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('ATM Collection Service'),
          content: const Text(
            'ATM Collection is a service for collecting cash from ATMs. '
            'This service requires cash counting, photo documentation, and secure transport with proper security measures.',
          ),
          actions: [
            TextButton(
